...
class TransactionsView {
  async getAll(token: string, page: number): Promise<Transaction[]> {
    try {
      ...
      const transactions: Transaction[] = await TransactionsDatabase.get(page);
  ...
  async create(user_id: string, value: number, type: string, description: string, token: string): Promise<string> {
    ...
    try {
      ...
      await TransactionsDatabase.create(id, user_id, value, type as Types, description);
