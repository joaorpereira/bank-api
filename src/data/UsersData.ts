...
class UserDatabase extends DataBase {
  ...
  async get(id: string): Promise<User> {
    try {
      const response = await DataBase.connection.raw(`SELECT * FROM ${this.tableName} WHERE id="${id}"`);
      return response[0][0];
    } catch (error: any) {
      throw new Error(error.sqlMessage || error.message);
    }
  ...
  async getByEmail(email: string): Promise<User> {
    try {
      const response = await DataBase.connection.raw(`SELECT * FROM ${this.tableName} WHERE email="${email}"`);
      return response[0][0];
    } catch (error: any) {
      throw new Error(error.sqlMessage || error.message);
    }
  ...
  async delete(id: string): Promise<void> {
    try {
      await DataBase.connection.raw(`DELETE FROM ${this.tableName} WHERE id="${id}";`);
    } catch (error: any) {
      throw new Error(error.sqlMessage || error.message);
    }
  }
